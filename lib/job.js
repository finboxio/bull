...
function addJob(queue, job) {
  ...
  const execPipeline = pipeline && opts.execPipeline;
  delete job.opts.pipeline;
  delete job.opts.execPipeline;

  const jobData = job.toData();

  return scripts.addJob(pipeline || queue.client, queue, jobData, {
