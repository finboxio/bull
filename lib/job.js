...
Job.create = function(queue, name, data, opts) {
  ...
  return scripts.addJob(queue, queue.client, job)
    .then((jobId) => {
      job.id = jobId
      debuglog('Job added', job.id);
      return job;
    });
};

Job.createBulk = function(queue, jobs, opType = 'multi') {
  jobs = jobs.map(job => new Job(queue, job.name, job.data, job.opts));
  return scripts.addManyJobs(queue, jobs, opType)
    .then((jobIds) => {
            ...
            if (finished) {
              Job.fromId(this.queue, this.id)
                .then(job => {
                  ...
                  debuglog('error resolving finished job', e)
                });
            } else {
              this.getState()
                .then((state) => {
                ...
                .catch((e) => {
                  debuglog('error checking state', e)
                })
            }
          });
        }
