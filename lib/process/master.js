...
process.on('message', msg => {
  switch (msg.cmd) {
    case 'init':
      import(msg.file)
        .then(({ default: handler }) => handler)
        .catch((e) => {
          const handler = require(msg.file);
          return handler.default || handler;
        })
        .then((handler) => {
          if (msg.options) handler = handler(msg.options);
          if (handler.length > 1)  {
            processor = promisify(handler);
          } else {
            processor = function() {
              try {
        ...
        .then(() => {
          status = 'IDLE';
          process.send({ cmd: 'init-complete' });
        })
