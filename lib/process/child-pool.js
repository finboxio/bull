...
ChildPool.prototype.retain = function(processFile, options) {
  const _this = this;
  ...
  return convertExecArgv(process.execArgv).then(execArgv => {
    ...
    return initChild(child, child.processFile, child.options).then(() => {
      return child;
    });
  });
};
...
async function initChild(child, processFile, options) {
  ...
  await new Promise(resolve =>
    child.send({ cmd: 'init', file: processFile, options }, resolve)
  );
  await onComplete;
}
