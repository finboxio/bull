    ...
    if (type === 'client') {
      const initialize = () => {
        const p = new Promise((resolve, reject) => {
          ...
          client.once('ready', () => {
            p.$connecting = false
            debuglog('client ready, reloading commands')
            commands(client)
              .then(() => debuglog(name + ' queue ready'))
              .then(() => resolve())
              .catch((e) => {
      ...
      client.on('reconnecting', () => {
        if (!this._initializing?.$connecting) {
          this._initializing = initialize()
        }
      })
    }
...
Queue.prototype.close = function(doNotWaitJobs) {
  ...
  return (this.closing = this.isReady()
    ...
    .catch((e) => debuglog('cleanup error', e))
...
/**
  ...
  @method process
*/
Queue.prototype.process = function(name, concurrency, handler, options) {
  switch (arguments.length) {
    case 1:
      ...
      break;
    case 2: // (string, function) or (string, string) or (number, function) or (number, string) or (string, object)
  ...
  this.setHandler(name, handler, options);
...
Queue.prototype.setHandler = function(name, handler, options) {
  if (!handler) {
  ...
  if (typeof handler === 'string') {
    const supportedFileTypes = ['.js', '.ts', '.flow', '.mjs', '.cjs'];
    const processorFile =
      handler +
    ...
    this.handlers[name] = sandbox(handler, options, this.childPool).bind(this);
  } else {
...
/**
  ...
  @param {string} [opType=multi] - The bulk operation type to use. Possible values are multi and pipeline. Defaults to multi.
*/
Queue.prototype.addBulk = function(jobs, opType = 'multi') {
  ...
  return Job.createBulk(this, decoratedJobs, opType);
};
/**
  Empties the queue.
...
*/
Queue.prototype.empty = function(delOpts = {}) {
  ...
  const { opType = 'multi', batchSize = 10000 } = delOpts
  return multi.exec().then(res => {
    ...
    if (jobKeys.length || limited.length) {
      const op = this.client[opType]();

      for (let i = 0; i < jobKeys.length; i += batchSize) {
        op.del.apply(op, jobKeys.slice(i, i + batchSize));
      }

      for (let i = 0; i < limited.length; i += batchSize) {
        op.del.apply(op, limited.slice(i, i + batchSize));
      }

      return op
        .exec()
        .then(r => r.reduce((t, [e, n]) => [e || t[0], n + t[1]], [null, 0]))
        .then(([e, n]) => {
          if (e) throw e;
          return n;
        });
...
Queue.prototype.whenCurrentJobsFinished = function() {
  ...
  // Force reconnection of blocking connection to abort blocking redis call immediately.
  //
  const forcedReconnection = redisClientDisconnect(this.bclient)
    .then(() => this.bclient.connect())
    .catch((e) => {})

  return Promise.allSettled(this.processing).then(() => forcedReconnection);
};
