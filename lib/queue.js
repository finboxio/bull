...
*/
Queue.prototype.add = function(name, data, opts, pipeline) {
  if (typeof name !== 'string') {
    pipeline = opts;
    opts = data;
    data = name;
...
Queue.prototype.addMany = function(jobs) {
  let pipeline = this.client.pipeline();
  const created = jobs.map((job, i) => {
    const { name = Job.DEFAULT_JOB_NAME, data, opts = {} } = job;
    if (i === jobs.length - 1) opts.execPipeline = true;
    const created = this.add(name, data, opts, pipeline);
    return created;
  });
...
Queue.prototype.empty = function() {
  ...
  return multi.exec().then(res => {
    ...
    if (jobKeys.length || limited.length) {
      multi = this.multi();
      while (jobKeys.length) {
        multi.del.apply(multi, jobKeys.splice(0, 100000));
      }
      while (limited.length) {
        multi.del.apply(multi, limited.splice(0, 100000));
      }
      return multi
        .exec()
