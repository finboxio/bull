...
const scripts = {
  isJobInList(queue, listKey, jobId) {
    return queue.client.isJobInList([listKey, jobId]).then(result => {
      return result === 1;
    });
  },

  addJob(queue, client, job) {
    const opts = job.opts;
    ...
    const args = [
      queueKeys[''],
      _.isUndefined(opts.jobId) ? '' : opts.jobId,
      jobData.name,
      jobData.data,
      jobData.opts,
      jobData.timestamp,
      jobData.delay,
      jobData.delay ? jobData.timestamp + jobData.delay : 0,
      opts.priority || 0,
  ...
  addManyJobs(queue, jobs, opType) {
    const op = queue.client[opType]()
    jobs.forEach((job) => scripts.addJob(queue, op, job))
    return op.exec().then((res) => {
      res.forEach((res, index) => {
        if (res[0] instanceof Error) {
          console.error(process.pid, 'Error on bulk create', res[0])
          throw res[0]
        }
        jobs[index].id = res[1];
      });
      ...
      return new Promise(async (resolve, reject) => {
        while (!done) {
          try {
            ...
            if (err.message.match(/BUSY|NOSCRIPT|ECONNRESET|ECONNREFUSED|ENOTFOUND/)) {
              debuglog(`${script} failed, retrying...`, err)
              const queue = args[0]?.queue || args[0]
              const waits = [
                queue?.isReady?.(),
                new Promise((res) => setTimeout(res, Math.min(Math.pow(2, attempts++) * backoff, maxBackoff)))
              ]
